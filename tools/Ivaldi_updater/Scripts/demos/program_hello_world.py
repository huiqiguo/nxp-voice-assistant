...
Brief
+++++
** Test script for blhost and sdphost wrappers. Programs example app on i.mx RT device. **
...
def main():
    """
        Erase 0x2000000 of flash starting at address 0x60000000
        Program /Image_Binaries/hello_world_vcom.bin into flash starting at address 0x60000000
        Read initial PC from Vector table (at 0x60002004) and launch application
    ...
    if bl.write_memory('0x60000000', '../../Image_Binaries/hello_world_vcom.bin')['ret']:
    ...
    if read_entry_resp['ret']:
        print('ERROR: Could not read entry point from memory!')
        sys.exit(1)
    else:
    ...
    stack_pnt = 0
    if read_sp_resp['ret']:
        print('ERROR: Could not read stack pointer from  memory!')
        sys.exit(1)
    else:
