...
void appTask(void *arg)
{
    ...
    else
    {
        ...
        xResult = xMQTTAgentInit(2048, (tskIDLE_PRIORITY + 2));

        if (xResult == pdPASS)
        {
            xResult = xTaskCreate(ota_task, "OTA", 384, NULL,
                    (tskIDLE_PRIORITY + 1), &otaTaskHandle);
        }
    }
#endif /* ENABLE_AWS_OTA */
#endif /* ENABLE_WIFI */
    ...
    while (1)
    {
        BaseType_t notification_status = xTaskNotifyWait(0xffffffffU, 0xffffffffU, &taskNotification, 50);

        if (notification_status == pdPASS)
        {
