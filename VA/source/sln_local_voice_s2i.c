...
/* Used models */
#include "VIT_Model_en_Audioplayer.h"
...
SDK_ALIGN(uint8_t __attribute__((section(".bss.$SRAM_OC_CACHEABLE"))) s_vitSlowMemory[SLOW_MEMORY_SIZE_BYTES], 8);
...
static VIT_ReturnStatus_en VIT_Init(void)
{
    ...
    {
        case ASR_VA:
            VIT_Status = VIT_SetModel((const PL_UINT8 *)VIT_Model_en_audioplayer, VIT_MODEL_IN_FAST_MEM);
            break;
    }
...
void local_voice_task(void *arg)
{
    ...
    if ((appAsrShellCommands.status != WRITE_SUCCESS) ||
        (appAsrShellCommands.demo != ASR_VA) ||
